...
class HapusPendudukAuth extends HapusPenduduk
{
    public function index()
    {
        if (!empty($_POST)) {
            ...
            if ($this->model('PendudukModel')->cekPenerimaanBansos($data) == 0 && ($data['tipeAkun'] != 3 || $data['tipeAkun'] != 5)) {
            ...
            } else if ($data['tipeAkun'] == 3 || $data['tipeAkun'] == 5) {
                $data['nikDipengaruhi'] = $this->model('PendudukModel')->getNikArray($data)['nik'];
                $data['barisDipengaruhi'] += $this->model('PendudukModel')->hapusDataPenduduk($data);
            } else {
                $data['barisGagal']++;
            ...
            if ($data['barisGagal'] > 0) {
                $data['additionalMessage'] .= " Ada {$data['barisGagal']} baris yang tidak dapat dihapus karena sudah pernah menerima bansos.";
            }
