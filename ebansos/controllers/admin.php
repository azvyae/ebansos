...
class AdminAuth extends Admin
{
    ...
    public function generateBansos()
    {
        $data['user'] = $_SESSION['user'];
        if (!empty($_POST) && ($data['user']['tipeAkun'] == 5)) {
            $data['validity'] = $_SESSION['adminGenerator'] = hash('sha256', time());
            $data['namaBansos'] = $_POST['namaBansos'];
            $data['nama'] = $data['namaBansos'];
            $data['userId'] = implode(explode(' ',$data['namaBansos']));
            $data['password'] = $this->generatePassword();
            $data['tipeAkun'] = 4;
            $data['rw'] = null;
            $data['rt'] = null;
            ...
            if ($this->model('UserModel')->getUserId($data['userId']) == 0) {
                if ($this->model('UserModel')->cekUserBansos($data) < 3) {
                    $arrayData[0] = $this->tambahkanUser($data['nama'], $data['userId'], $data['password'], $data['tipeAkun'], $data['rw'], $data['rt']);
                } else {
                    $data['additionalMessage'] = ' karena jumlah petugas/acara bansos sudah lebih dari 3, silakan hapus terlebih dahulu';
                }
            } 
            if (!empty($arrayData)) {
                $this->registerRiwayat($data['user'], 'Membuat Event Bansos', 'Kegiatan ' . $data['nama']);
                Flasher::setFlash('Anda berhasil', 'menambahkan ' . count($arrayData) . ' acara ' . $data['nama'], 'success');
            } else {
                Flasher::setFlash('Anda gagal', 'menambahkan acara bansos' . $data['additionalMessage'], 'danger');
            }
            $this->printUserAndPassword($data['validity'], $arrayData);
        } else {
            header('Location: ' . BASEURL);
