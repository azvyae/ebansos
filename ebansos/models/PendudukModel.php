...
class PendudukModel
{
    ...
    {
        $this->table = 'penduduk';
        $this->table2 = 'tanggalterimabansos';
    ...
    public function getPenduduk($data)
    {
        if (!empty($data['hashId'])) {
            $query = "SELECT {$this->table}.*, {$this->table2}.tanggalMenerima, {$this->table2}.jenisBansos FROM {$this->table} LEFT JOIN {$this->table2} ON {$this->table}.hashId = {$this->table2}.hashId WHERE {$this->table}.hashId = :hashId ORDER BY {$this->table2}.tanggalMenerima DESC LIMIT 0, 1";
            $this->db->query($query);
            $this->db->bind('hashId', $data['hashId']);
        } else {
            $query = "SELECT nik, nama, alamatRumah, {$this->table2}.tanggalMenerima, {$this->table2}.jenisBansos FROM {$this->table} LEFT JOIN {$this->table2} ON {$this->table}.hashId = {$this->table2}.hashId  WHERE nik = :nik AND tanggalLahir = :tanggalLahir ORDER BY {$this->table2}.tanggalMenerima DESC LIMIT 0, 1";
            $this->db->query($query);
    ...
    public function cekPenerimaanBansos($data)
    {
